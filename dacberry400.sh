...
osreleases=( "Raspbian" "Debian" ) # list os-releases supported
...
gitreponame="DACBerry-400" # the name of the git project repo
...
enable_pi_i2c() {
    if grep -q "#dtparam=i2c_arm=on" $CONFIG; then
        sudo sed -i "/^#dtparam=i2c_arm=on$/ s|#||" $CONFIG
        ASK_TO_REBOOT=true
    fi
}
...
enable_pi_i2s() {
    if grep -q "#dtparam=i2s=on" $CONFIG; then
        sudo sed -i "/^#dtparam=i2s=on$/ s|#||" $CONFIG
        ASK_TO_REBOOT=true
    fi
}
